...
export interface Session {
  ...
  skipped_dimensions: ProfileDimension[]; followup_counts: Partial<Record<ProfileDimension, number>>; current_dimension: ProfileDimension|null
...
export interface Profile { id:string; user_id:string; session_id:string; version:number; objective_profile:ObjectiveProfile; archetype_code:string; archetype_title:string; risk_level:'conservative'|'moderate'|'growth'; loss_tolerance_percent:number; confidence:number; completeness:number; education_only:boolean; dimension_scores:Record<string,number|null>; profile_evidence:Partial<Record<ProfileDimension,number>>; report_summary:{traits:string[];risk_notice:string;reasoning:string[];low_confidence:ProfileDimension[]}; style_code:string; style_logic:string; style_name:string; style_summary:string; master_name:string; master_name_en:string; master_match_reason:string }
export interface ProfileWithRecommendations { profile: Profile; recommendations: Recommendation[] }
export interface AISetting { id:string|null; capability:Capability; provider:string; model_name:string; base_url:string|null; api_key_configured:boolean; prompt_version:string; min_rounds:number; max_rounds:number; enabled:boolean; version:number }
export interface EditableAISetting extends AISetting { prompt_content:string }
