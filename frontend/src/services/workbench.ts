...
export function validOrigin(raw:string|undefined):string|null{if(!raw||raw==='*')return null;try{const url=new URL(raw);if(!['http:','https:'].includes(url.protocol))return null;return url.origin}catch{return null}}
...
export function resolveWorkbenchTarget(host:WindowHost):MessageTarget|null{try{if(host.parent&&host.parent!==host)return host.parent;if(host.opener)return host.opener}catch{return null}return null}
