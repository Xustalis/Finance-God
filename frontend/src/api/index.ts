...
import type { AISetting, AuthData, EditableAISetting, EvidenceConfirmation, MessageTurn, ObjectiveProfile, ProfileDimension, ProfileWithRecommendations, Session, User } from '@/types/api'
...
export const onboardingApi = {
  ...
  sendMessage:(id:string,body:{request_id?:string;content?:string;input_mode?:'text'|'voice';confirm_pending?:boolean})=>api.post<MessageTurn|EvidenceConfirmation>(`/onboarding/sessions/${id}/messages`,body),
...
export const adminApi = { list:()=>api.get<AISetting[]>('/admin/ai-settings'), update:(body:EditableAISetting)=>api.put<AISetting>('/admin/ai-settings',adminUpdatePayload(body)), test:(body:{capability:string;provider:string;model_name:string})=>api.post<{ok:boolean;adapter:string;credential_status:string}>('/admin/ai-settings/test',body) }
