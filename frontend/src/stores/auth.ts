...
export const useAuthStore=defineStore('auth',()=>{
  ...
  async function devLogin(){loading.value=true;try{persist(await authApi.devLogin())}finally{loading.value=false}}
  ...
  function logout(){token.value=null;user.value=null;localStorage.removeItem(TOKEN);localStorage.removeItem(USER);localStorage.removeItem('finance-god-profile-completed')}
  return{token,user,loading,authenticated,isAdmin,login,devLogin,register,hydrate,updateProfile,logout}
})
