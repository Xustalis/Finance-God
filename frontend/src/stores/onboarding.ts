...
export const useOnboardingStore=defineStore('onboarding',()=>{
  ...
  let pendingConfirmation:{accepted:boolean;requestId:string}|null=null
  ...
  function clearMemory(){objective.value={};objectiveIndex.value=0;messages.value=[];pendingContent=null;pendingConfirmation=null}
  ...
  async function sendContent(content:string,input_mode:InputMode='text'){if(!session.value)throw new Error('会话尚未建立');if(session.value.pending_profile_evidence)throw new Error('请先确认或不采用上一条判断');busy.value=true;error.value='';const retry=pendingContent?.content===content&&pendingContent.inputMode===input_mode?pendingContent:{content,inputMode:input_mode,requestId:makeId()};pendingContent=retry;try{const result=await client.sendMessage(session.value.id,{request_id:retry.requestId,content,input_mode});pendingContent=null;if('turn' in result){session.value=result.session;messages.value.push({role:'user',content:result.user_message.content},{role:'assistant',content:result.assistant_message.content});persistMessages()}return result}catch(e){error.value=e instanceof Error?e.message:'对话暂时不可用';throw e}finally{busy.value=false}}
  async function confirmEvidence(accepted:boolean){if(!session.value?.pending_profile_evidence)throw new Error('当前没有待确认判断');busy.value=true;const retry=pendingConfirmation?.accepted===accepted?pendingConfirmation:{accepted,requestId:makeId()};pendingConfirmation=retry;try{const result=await client.sendMessage(session.value.id,{request_id:retry.requestId,confirm_pending:accepted});pendingConfirmation=null;session.value=result.session;return result}finally{busy.value=false}}
  ...
  return{session,objectiveIndex,objective,messages,busy,error,currentObjective,objectiveComplete,configureApi,restore,reset,persistMessages,selectObjective,previousObjective,submitObjective,sendContent,confirmEvidence,skipCurrent,complete}
})
