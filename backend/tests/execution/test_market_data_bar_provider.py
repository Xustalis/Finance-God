...
class _MarketData:
    ...
    def read_bars(self, _symbol: str, *, limit: int):
        assert limit == 500
...
def test_provider_selects_the_first_real_bar_after_submission() -> None:
    ...
    provider = MarketDataBarProvider(_MarketData((before, later, first_after)))
    ...
    assert result.evidence.version == first_after.provider_time
...
def test_provider_does_not_invent_staleness_for_unknown_upstream_freshness() -> None:
    ...
    provider = MarketDataBarProvider(
        _MarketData(
            (_bar(submitted_at + timedelta(minutes=1), freshness="unknown"),)
        )
    )
    draft = SimpleNamespace(
        instrument_id="000001.SZ",
